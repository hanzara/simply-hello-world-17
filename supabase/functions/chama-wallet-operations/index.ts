...
async function handleTopUp(supabase: any, member: any, amount: number) {
  ...
  if (centralWallet.balance < amount) {
    throw new Error(`Insufficient balance. Available: ${centralWallet.balance}`);
  }
  ...
  await supabase.from('chama_audit_trail').insert({
    ...
    amount: amount,
    details: { from: 'central_wallet', to: 'mgr_wallet' }
  });

  return { success: true, message: `Successfully topped up ${amount} to MGR wallet`, newBalance: member.mgr_balance + amount };
}
